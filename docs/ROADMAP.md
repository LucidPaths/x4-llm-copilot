...
- Added raw-first live trade probe routing for `scope:"docked_station"`: MD passes the fetch JSON into Lua, Lua branches on `intent:"trade_in_sector"`, emits `schema:"trade_offers_probe_v1"`, and Python preserves `offers_raw` / `nontrade_offers_raw`. Live bytes at `VIG Ice Refinery I` confirmed a list of offer objects; Python now normalizes observed fields while preserving each full raw offer under `raw`. `scope:"radar_range"` is reserved for the future multi-station reader and fails closed today.
...
- Remaining in v0.2: implement `scope:"radar_range"` multi-station trade reads, broader trade-shape validation across more stations, sector objects, faction relation snapshots, and cleanup of duplicate idle retry loop instances.
